Param(
    ...
    [int]$year,
    [Parameter(Mandatory = $true,
        HelpMessage = "You must provide a month")]
    [int]$month

)
# changed name to 
sqlcmd -d babblefish -o error.log -E  -i "C:\Users\me\Source\Repos\BabbleFishV4\BabbleFishV3\sql_querys\ProcessAttachments_AT_paramsByMonth.sql" -v p_year=$year p_month=$month --% -S daffY-duck
...
bcp "select attachid, [external id], [internal id], caseNumberref, transactionHandle,title,typeRef, directionRef, memo, filename, attachdate, attachtime,run_id from babblefish.dbo.ATTACHMENT_CSV;" queryout $EXPORT_TXT_FILENAME -d $EXPORT_DB -T -c -t ',' >> .\attachment_csv.log
