...
#modified 4/26/2020 to process attachment by month, not quarters
...
function Invoke-Attachments() {
  ...
  for ($year = 2015; $year -lt 2021; $year++) {
    for ($month = 1; $month -lt 13; $month++) {
      Clear-Host
      Write-Host "Processing...." $year $month -foreground Green
      Powershell -Command "C:\Users\me\Source\Repos\BabbleFishV4\BabbleFishV3\powershell\CreateCSV_ATTACHMENT_AT_paramsByMonth.ps1 $year $month"
    }
  }
