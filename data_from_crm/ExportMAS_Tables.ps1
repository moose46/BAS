...
function Export-TablesToCSV($tableName) {
        if( $tableName -eq "SO_SalesOrderHeader"){
            $query = "select * from $tableName where isnumeric(SO_SalesOrder)"
        }
        else {
        ...
        $header = $header + $ds.Tables[0].Columns -join ","
        ...
        for ($i = 0; $i -lt $ds.Tables[0].Rows.Count; $i++) {
            $rowData = ""
            for ($j = 0; $j -lt $cols; $j++) {
                $rowData = $rowData + "$($ds.Tables[0].Rows[$i][$j] -replace ',',''),"
            }
